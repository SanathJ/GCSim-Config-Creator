...
def makeCharConfig(c):
    with sqlite3.connect('configs.db') as con:
        ...
        artifacts = cursor.execute('''
            ...
            WHERE id IN (?, ?, ?, ?, ?)
            ''',
            (row['flower'], row['plume'], row['sands'], row['goblet'], row['circlet'],)).fetchall()
        ...
        for artifact in artifacts:
            ...
            if util.GOODStatToSimStat(artifact['mainStat']) not in mainStats.keys():
                mainStats[util.GOODStatToSimStat(artifact['mainStat'])] = util.artifact_stats[str(artifact['rarity'])][artifact['mainStat']][artifact['level']]
            else:
                mainStats[util.GOODStatToSimStat(artifact['mainStat'])] += util.artifact_stats[str(artifact['rarity'])][artifact['mainStat']][artifact['level']]
            ...
            for substat in json.loads(artifact['substats'])['substats']:
                ...
                if util.GOODStatToSimStat(substat['key']) not in substats.keys():
                    substats[util.GOODStatToSimStat(substat['key'])] = substat['value'] / divider
                else:
                    substats[util.GOODStatToSimStat(substat['key'])] += substat['value'] / divider
